...
g0[1:6] <- 2.991 # prior prob 0.8-0.99 middle 95%
s0 <- diag(lags + 1) * 1.526^2
for (s in 1:6)
  s0[s, s] <- 0.819^2
ts0 <- rep(1, lags)
ts0[1:6] <- 10 # higher prob of split in first 6 lags
ts0 <- ts0 / sum(ts0)

mlist <- 
  foreach(m = 1:restarts, .errorhandling = "remove", 
         .packages = "dlmtree",
         .export = c("summer_data", "summer_temp_dat", "temp_splits",
                     "g0", "s0", "ts0"),
         .verbose = FALSE) %dopar%
  tdlnm(logdeath ~ as.factor(year) * as.factor(month) + as.factor(dow),
        ...
        monotone = TRUE, verbose = F,
        monotone.gamma0 = g0, monotone.sigma = s0,
        time.split.prob = ts0, monotone.time.kappa = 2)
...
g0 <- rep(0, lags + 1)#-2.298 + 2*2.298/(1+exp((-10:10)/2))
s0 <- diag(lags + 1) * 1.502^2
ts0 <- rep(1, lags + 1)
ts0 <- ts0 / sum(ts0)
...
mlist <- 
  foreach(m = 1:restarts, .errorhandling = "remove", 
         .packages = "dlmtree",
         .export = c("winter_data", "winter_temp_dat", "w_temp_splits",
                     "g0", "s0", "ts0"),
         .verbose = FALSE) %dopar%
  tdlnm(logdeath ~ as.factor(year) * as.factor(month) + as.factor(dow),
        data = winter_data,
        ...
        monotone = TRUE, verbose = F,
        monotone.gamma0 = g0, monotone.sigma = s0,
        time.split.prob = ts0, monotone.time.kappa = 1)
...
par(mfrow = c(1, 2))
boxplot(1/(1+exp(-winter_deaths$zirtGamma)), ylim = c(0, 1), main = "Prob. nonzero ERC", names = 0:lags); points(1/(1+exp(-g0-1.96*sqrt(diag(s0)))), col = "red", pch = "-", cex = 3); points(1/(1+exp(-g0+1.96*sqrt(diag(s0)))), col = "red", pch = "-", cex = 3); 
boxplot(winter_deaths$timeProbs, ylim = c(0, 1), main = "Prob. time split", names = sapply(1:lags, function(i) paste0(i-1, "/", i))); points(ts0, col = "blue", pch = 16)
```
